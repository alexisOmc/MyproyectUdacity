...
struct UserData: Codable {
    ...
    let mailingAddress: JSONNull?
    let cohortKeys: [JSONAny]
    let signature, stripeCustomerID: JSONNull?
    let userDataGuard: Guard
    let facebookID, timezone, sitePreferences, occupation: JSONNull?
    let image: JSONNull?
    let firstName: String
    let jabberID, languages: JSONNull?
    let badges: [JSONAny]
    let location, externalServicePassword: JSONNull?
    let principals, enrollments: [JSONAny]
    let email: Email
    let websiteURL: JSONNull?
    let externalAccounts: [JSONAny]
    let bio, coachingData: JSONNull?
    let tags, affiliateProfiles: [JSONAny]
    let hasPassword: Bool
    let emailPreferences, resume: JSONNull?
    let key, nickname: String
    let employerSharing: Bool
    let memberships: [JSONAny]
    let zendeskID: JSONNull?
    let registered: Bool
    let linkedinURL, googleID: JSONNull?
    let imageURL: String
